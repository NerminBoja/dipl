...
public class MongoAccess {
    ...
    public static DB getDB() {
        ...
        DB db = client.getDB(MongoConstants.MONGO_DATABASE);

        return db;
    }

    public static boolean saveUser(User user) {
        DBCollection users = getDB().getCollection(MongoConstants.USERS);
        if (users.find(user).hasNext()) {
            return false;
        } else {
            users.save(user);
            users = getDB().getCollection(MongoConstants.USERS);
            if (users.find(user).hasNext()) {
                return true;
            } else {
                return false;
    ...
    public static boolean addLock(Lock lockToAdd) {
        DBCollection locks = getDB().getCollection(MongoConstants.LOCKS);
        BasicDBObject find = new BasicDBObject();
        ...
        if (locks.find(find).hasNext()) {
            ...
            locks = getDB().getCollection(MongoConstants.LOCKS);
    ...
    public static boolean addLocation(Location locationToAdd) {
        DBCollection locations = getDB().getCollection(MongoConstants.LOCATIONS);
        locations.insert(locationToAdd);
        locations = getDB().getCollection(MongoConstants.LOCATIONS);
    ...
    public static List<UserLock> getLocksForUser(String userName) throws IOException {
        DBCollection userLockCollection = getDB().getCollection(MongoConstants.USER_LOCK);
    ...
    public static boolean addUserToLock(UserLock userLock) {
        DBCollection collection = getDB().getCollection(MongoConstants.USER_LOCK);
        BasicDBObject find = new BasicDBObject();
    ...
    public static boolean removeLocation(LocationAndroid location) throws IOException {
        DB db = getDB();
        DBCollection locationCollection = db.getCollection(MongoConstants.LOCATIONS);
        DBCollection lockCollection = db.getCollection(MongoConstants.LOCKS);
    ...
    public static boolean removeLock(LockAndroid lock) {
        DBCollection lockCollection = getDB().getCollection(MongoConstants.LOCKS);
        BasicDBObject lockFind = new BasicDBObject();
    ...
    public static boolean removeUser(String username) {
        DB db = getDB();
        DBCollection users = db.getCollection(MongoConstants.USERS);
        BasicDBObject find = new BasicDBObject();
        find.append("userName", username);
        if (!users.find(find).hasNext()) {
            return false;
